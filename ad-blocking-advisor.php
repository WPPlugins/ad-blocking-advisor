...
DEFINE("AD_BLOCKING_ADVISOR_VER", "1.4");
...
class Ad_Blocking_Advisor {
	...
	public function __construct() {
		...
		add_action( 'wp_footer', array( &$this, 'footerPNG' ), 1 );
	...
	public function install() {
        ...
        //update_option( "aba_timer_allow", "true", true );
		//update_option( "aba_timer_timeout", "1000", true );
	}
	...
	public function upgrade_options() {
		...
		// Changes for versions below 1.3
		if ( $aba_version < '1.3' ) {
			// 1.3 adds the ability to delay the banner from showing
            // Removed these options in 1.4
			//update_option( "aba_timer_allow", "true", true );
			//update_option( "aba_timer_timeout", "1000", true );
		}
	...
	public function load_scripts() {
		...
		// Removed in 1.1 since plugin updates just delete the file if it exists anyways. :/
		// Only load the custom frontend CSS file if it exists
		//$custom_frontend_css = plugin_dir_path( __FILE__ ) . 'css/frontend_custom.css';
		//if ( file_exists( $custom_frontend_css ) ) {
			//wp_register_style( 'aba_frontEndCSS_custom' , plugins_url( 'css/frontend_custom.css', __FILE__), array( 'aba_frontEndCSS' ) ); // This loads both styles, with a dependancy
		//	wp_register_style( 'aba_frontEndCSS_custom' , plugins_url( 'css/frontend_custom.css', __FILE__) );
		//	wp_enqueue_style( 'aba_frontEndCSS_custom' );
		//}
	}
	...
	public function footerPNG() {
		// Put our fake ad in the footer. This is a 1px transparent image. We use the div class to know if ad blocking software is being used or not.
		echo '<div class="footer_ad">';
	...
	public function alert_banner() {
		...
		if ( $enabled ) {
			...
			';
                ...
                // Remove in 1.4, but left in for archive
				// if ( $timerAllow ) {
					// $html .= '
						// setTimeout(function() { 
					// ';
	...
	public function setup_settings() {
        ...
        // Removed in 1.4, but left in for archive
		// add_settings_field(
			// 'aba_timer_allow',										// ID of the field
			// __('Enable Detection Delay Timer:','ad-blocking-advisor'),		// Title
			// array( &$this , 'timer_allow' ),						// Callback function
			// 'aba_general_settings',									// Which page should this option show on
			// 'aba_general_settings_section'							// Section name to attach to
		// );
		
		// add_settings_field(
			// 'aba_timer_timeout',										// ID of the field
			// __('Detection Delay Timer (in milliseconds):','ad-blocking-advisor'),		// Title
			// array( &$this , 'timer_timeout' ),							// Callback function
			// 'aba_general_settings',										// Which page should this option show on
			// 'aba_general_settings_section'								// Section name to attach to
		// );
        ...
        // Removed in 1.4, but left in for archive
		//register_setting( 'aba_general_settings', 'aba_timer_allow' );
		//register_setting( 'aba_general_settings', 'aba_timer_timeout' );		
    ...
    // Removed as of 1.4, but left in for archive
	// public function timer_allow() {
		
		// $option = get_option('aba_timer_allow');
				
		// echo '
			// <input type="checkbox" id="aba_timer_allow" name="aba_timer_allow" value="true" '. checked( $option, "true", false ) .' >
			// <label for="aba_timer_allow"><span class="description">' . __( 'Default: unchecked (disabled)<br>Enable or disable the display timer. Even though the ad block detection runs when the site has finished loading, some browsers will not show the advisor banner correctly. This is typically due to a timing issue on how some browsers load websites. The display timer tells the browser to wait until after the site is done loading to display the banner.', 'ad-blocking-advisor') . '</span></label>
		// ';
	// }
	
	// public function timer_timeout() {
		
		// $option = get_option('aba_timer_timeout');
				
		// echo '
			// <input type="text" id="aba_timer_timeout" name="aba_timer_timeout" autocomplete="off" value="'. $option .'">
			// <label for="aba_timer_timeout"><span class="description">' . __( 'Default: 1000<br>Change the timer (in milliseconds) to show the banner.<br><strong>Note: Configure this timer delay so that your users do not miss the banner, but still allow browsers to finish loading.</strong> You can choose any time delay like 500 milliseconds (1/2 second), or 5000 milliseconds (5 seconds), it is up to you.','ad-blocking-advisor') .'</a></span></label>
		// ';
	// }
